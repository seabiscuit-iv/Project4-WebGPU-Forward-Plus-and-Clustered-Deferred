...
import { aspectRatio, canvas, device, fovYDegrees } from "../renderer";

class CameraUniforms {
    readonly buffer = new ArrayBuffer(32 * 4);
    private readonly floatView = new Float32Array(this.buffer);

    set viewProjMat([mat, inv_mat]: [Float32Array, Float32Array]) {
        this.floatView.set(mat.subarray(0, 16), 0);
        this.floatView.set(inv_mat.subarray(0, 16), 16);
    }
...
export class Camera {
    ...
    onFrame(deltaTime: number) {
        ...
        this.uniforms.viewProjMat = [viewProjMat, mat4.inverse(viewProjMat)];
